...
def wildArgs(**kwargs):
    wildArgs._id += 1
    lambda_kwargs_txt = [f'{k}={v}' for k, v in kwargs.items() if type(v) is not str]
    lambda_kwargs_txt += [f'{k}="{v}"' for k, v in kwargs.items() if type(v) is str]
    lambda_kwargs_txt = ','.join(lambda_kwargs_txt)
